...
> **Social Fit notu:** Bu skill Expo Router (`app/` dizini, `<Stack>`/`<Link>` from `expo-router`, `NativeTabs`) ve TypeScript varsayıyor. Social Fit mobile `@react-navigation` (native-stack + bottom-tabs) ve düz JavaScript kullanıyor — bu dosyadaki **"Navigation" ve "Routes" bölümlerini yoksay**. Styling (renk/shadow/safe-area genel kuralları) ve `references/` altındaki Expo SDK rehberleri (animations, controls, icons, visual-effects, gradients, media, storage) router'dan bağımsız, geçerli kalır.
